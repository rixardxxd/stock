...
from crawler.items.lhb_dfcf_item import LhbDfcfItem
from crawler.items.lhb_summary_dfcf_item import LhbSummaryDfcfItem

logger = logging.getLogger("LhbDfcfSpider")


class LhbDfcfSpider(Spider):
    """
    It's the class to crawl cinema info from mtime.com
    """
    name = "lhb_dfcf"
    # 不对重复的url进行过滤。这样可以保证summary table有全部的东西
    custom_settings = {
    ...
    def parse(self, response):
        ...
        for item in content['data']:
            ...
            summary_item = LhbSummaryDfcfItem()
            summary_item['lhb_date'] = item['Tdate']
            summary_item['stock_id'] = item['SCode']
            summary_item['stock_name'] = item['SName']
            summary_item['close_price'] = item['ClosePrice']
            summary_item['change_percent'] = item['Chgradio']
            summary_item['lhb_net_value'] = item['JmMoney']
            summary_item['lhb_buy_value'] = item['Bmoney']
            summary_item['lhb_sell_value'] = item['Smoney']
            summary_item['lhb_total_value'] = item['ZeMoney']
            summary_item['trade_amount'] = item['Turnover']
            summary_item['net_value_percent'] = item['JmRate']
            summary_item['total_value_percent'] = item['ZeRate']
            summary_item['turnover_ratio'] = item['Dchratio']
            summary_item['reason'] = item['Ctypedes']
            # 满足校验则生成新的url，产生新的请求
            url = detail_url_format.format(lhb_date, stock_id)
            yield Request(url, callback=self.parse_detail, meta={'summary_item': summary_item})
    ...
    def parse_detail(self, response):
        ...
        while index < len(div_reason):
            ...
            for tr in buy_list:
                ...
                lhb_detail_item['summary_item'] = response.meta['summary_item']
